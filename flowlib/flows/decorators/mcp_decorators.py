...
FlowClass = TypeVar("FlowClass", bound=type)
...
def mcp_tool(
    ...
    def decorator(cls: FlowClass) -> FlowClass:
        ...
        flow_cls = cast(
            FlowClass, flow(name=name, description=description, is_infrastructure=True)(cls)
...
def mcp_resource(
    ...
    def decorator(cls: FlowClass) -> FlowClass:
        # Apply standard flow decorator first (mark as infrastructure)
        flow_cls = cast(FlowClass, flow(description=description, is_infrastructure=True)(cls))  # type: ignore[arg-type]
...
def mcp_client_aware(client_name: str) -> Callable[[FlowClass], FlowClass]:
    ...
    def decorator(cls: FlowClass) -> FlowClass:
        ...
        async def get_mcp_client(self: Any) -> Optional["MCPClientProvider"]:
            ...
            if not self._mcp_client:
                ...
                try:
                    provider = await provider_registry.get_by_config(self._mcp_client_name)
                    self._mcp_client = cast(Optional["MCPClientProvider"], provider)
        ...
        async def call_mcp_tool(self: Any, tool_name: str, arguments: dict[str, Any]) -> Any:
...
def is_mcp_tool(flow_class: Any) -> bool:
    """Check if a flow is marked as an MCP tool."""
    return hasattr(flow_class, "__mcp_metadata__") and (
        "expose_as_tool" in flow_class.__mcp_metadata__
        and flow_class.__mcp_metadata__["expose_as_tool"]
    )


def is_mcp_resource(flow_class: Any) -> bool:
    """Check if a flow is marked as an MCP resource."""
    return hasattr(flow_class, "__mcp_resource_metadata__") and (
        "expose_as_resource" in flow_class.__mcp_resource_metadata__
        and flow_class.__mcp_resource_metadata__["expose_as_resource"]
    )
