...
class ATypedFlow:
    ...
    async def process(self, data: TextInput) -> dict[str, Any]:
        ...
        return {"processed": data.text.upper(), "options_used": data.options}

    async def format_output(self, data: dict[str, Any]) -> TextOutput:
        """Format the output data."""
        return TextOutput(result=data["processed"], metadata={"options": data["options_used"]})
...
class AnotherTypedFlow:
    ...
    async def process(self, data: TextInput) -> dict[str, Any]:
        """Process the input data."""
        return {"processed": data.text.lower(), "options_used": data.options}
...
class CombinedFlow(Flow):
    ...
    async def run_all(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Retrieve flows from registry instead of instantiating
        simple_flow = flow_registry.get_flow("ATypedFlow")
        typed_flow = flow_registry.get_flow("AnotherTypedFlow")

        # We can only call execute() on the flow instances
        simple_result = await simple_flow.execute(Context(data=data))

        # Prepare input for the typed flow
        typed_input = TextInput(
            text=simple_result.data["result"], options={"from_simple_flow": True}
        )
        ...
        # Combine results
        return {"simple_result": simple_result.data, "typed_result": typed_result.data}


# Example usage
async def run_examples() -> None:
    """Run the example flows."""
    # Example 1 - Retrieve flow from registry
    a_flow = flow_registry.get_flow("ATypedFlow")
    a_result = await a_flow.execute(Context(data={"text": "hello world"}))
    print(f"A Flow Result: {a_result.data}")

    # Example 2 - Retrieve flow from registry
    another_flow = flow_registry.get_flow("AnotherTypedFlow")
    ...
    # Example 3 - Retrieve combined flow from registry
    combined_flow = flow_registry.get_flow("CombinedFlow")
