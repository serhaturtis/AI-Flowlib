...
class StateTTLInfo(StrictBaseModel):
    ...
    ttl_seconds: int | None = Field(default=None, description="TTL in seconds for state storage")
    ...
    def extract_from_state(cls, state: AgentState) -> "StateTTLInfo":
        ...
        if (
            state.config
            and state.config.state_config
            and hasattr(state.config.state_config, "ttl_seconds")
        ):
...
class RedisStatePersisterSettings(ProviderSettings):
    ...
    redis_provider_name: str = Field(
        ..., min_length=1, description="The registered name of the RedisProvider instance to use."
    )
    key_prefix: str = Field(
        "agent_state:", description="Prefix for keys in Redis where state is stored."
    )
...
class RedisStatePersister(BaseStatePersister[RedisStatePersisterSettings]):
    ...
    def __init__(self, name: str, settings: RedisStatePersisterSettings):
        ...
        self._redis_provider: CacheProvider | None = None
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            if not provider.initialized:
                logger.debug(
                    f"Initializing underlying RedisProvider '{self.settings.redis_provider_name}'..."
                )
                await provider.initialize()
            self._redis_provider = provider
        except Exception as e:
            logger.error(
                f"Failed to initialize RedisStatePersister '{self.name}' with provider '{self.settings.redis_provider_name}': {e}",
                exc_info=True,
            )
    ...
    async def _save_state_impl(
        ...
        if not self.initialized or not self._redis_provider:  # Check Provider's flag
            ...
            raise RuntimeError(
                f"RedisStatePersister '{self.name}' is not initialized or redis provider is missing."
            )
        ...
        try:
            ...
            logger.debug(
                f"Saved state for task '{state_id}' to Redis key '{redis_key}' via persister '{self.name}'"
            )
            return True  # Indicate success
        except Exception as e:
            logger.error(
                f"Failed to save state '{state_id}' via persister '{self.name}': {e}", exc_info=True
            )
            return False  # Indicate failure

    # Renamed to _load_state_impl to match BaseStatePersister
    async def _load_state_impl(self, state_id: str) -> AgentState | None:
        ...
        try:
            ...
            logger.debug(
                f"Loaded state for task '{state_id}' from Redis key '{redis_key}' via persister '{self.name}'"
            )
            return state
        except Exception as e:
            logger.error(
                f"Failed to load state '{state_id}' via persister '{self.name}': {e}", exc_info=True
            )
    ...
    async def _delete_state_impl(self, task_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Deleted state for task '{task_id}' (key: '{redis_key}', count: {deleted_count}) via persister '{self.name}'"
            )
            return deleted_count > 0
        except Exception as e:
            logger.error(
                f"Failed to delete state '{task_id}' via persister '{self.name}': {e}",
                exc_info=True,
            )
            return False
    ...
    async def _shutdown(self) -> None:
        logger.info(
            f"Shutting down RedisStatePersister '{self.name}'. Underlying provider '{self.settings.redis_provider_name}' shutdown managed by registry."
        )
        self._redis_provider = None
...
class MongoStatePersisterSettings(ProviderSettings):
    ...
    mongo_provider_name: str = Field(
        ..., min_length=1, description="Registered name of the MongoDBProvider instance."
    )
    database_name: str = Field(
        ..., min_length=1, description="Name of the MongoDB database to use."
    )
    collection_name: str = Field(
        "agent_states", description="Name of the collection to store agent states."
    )


@provider(
    name="mongodb", provider_type="state_persister", settings_class=MongoStatePersisterSettings
)
...
class MongoStatePersister(BaseStatePersister[MongoStatePersisterSettings]):
    ...
    def __init__(self, name: str, settings: MongoStatePersisterSettings):
        super().__init__(name=name, settings=settings)
        self._mongo_provider: MongoDBProvider | None = None
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize MongoStatePersister '{self.name}': {e}", exc_info=True
            )
    ...
    async def _load_state_impl(self, state_id: str) -> AgentState | None:
        ...
        try:
            ...
            results = await self._mongo_provider.execute_query(
                collection=self.settings.collection_name, query={"_id": state_id}
            )
    ...
    async def _delete_state_impl(self, task_id: str) -> bool:
        ...
        try:
            ...
            result = await self._mongo_provider.delete_document(
                collection=self.settings.collection_name, query={"_id": task_id}
            )
...
class PostgresStatePersisterSettings(ProviderSettings):
    ...
    postgres_provider_name: str = Field(
        ..., min_length=1, description="Registered name of the PostgresProvider instance."
    )
    table_name: str = Field("agent_states", description="Name of the table to store agent states.")
    id_column: str = Field(
        "task_id", description="Name of the primary key column (holding the task_id)."
    )
    data_column: str = Field(
        "state_data", description="Name of the JSONB column holding the agent state."
    )


@provider(
    name="postgres", provider_type="state_persister", settings_class=PostgresStatePersisterSettings
)
# Corrected inheritance
class PostgresStatePersister(BaseStatePersister[PostgresStatePersisterSettings]):
    ...
    def __init__(self, name: str, settings: PostgresStatePersisterSettings):
        super().__init__(name=name, settings=settings)
        self._postgres_provider: DBProvider | None = None
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize PostgresStatePersister '{self.name}': {e}", exc_info=True
            )
    ...
    async def _load_state_impl(self, state_id: str) -> AgentState | None:
        ...
        try:
            ...
            """
            records = await self._postgres_provider.execute(select_sql, {"task_id": state_id})
            record = records[0] if records else None
            if record is None:
                return None
            if self.settings.data_column not in record:
                raise ValueError(
                    f"State data column '{self.settings.data_column}' missing from record for '{state_id}'"
                )
            state_data = record[self.settings.data_column]
            if state_data is None:
                raise ValueError(
                    f"State data column '{self.settings.data_column}' missing for '{state_id}'."
                )
    ...
    async def _delete_state_impl(self, task_id: str) -> bool:
        ...
        try:
            ...
            delete_sql = f"""
                ...
                WHERE {self.settings.id_column} = %(task_id)s
            """
            result = await self._postgres_provider.execute(delete_sql, {"task_id": task_id})
            logger.debug(f"Deleted state '{task_id}' via {self.name}. Result: {result}")
            return result is not None and result > 0  # Check if delete was successful
        except Exception as e:
            logger.error(f"Failed to delete state '{task_id}' via {self.name}: {e}", exc_info=True)
