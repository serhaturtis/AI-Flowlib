...
class TodoExecutionContext(StrictBaseModel):
    ...
    dependencies_met: bool = Field(
        default=True, description="Whether all dependencies are satisfied"
    )
...
class TodoExecutionResult(StrictBaseModel):
    ...
    tool_output: str | None = Field(default=None, description="Raw tool output")
    tool_errors: str | None = Field(default=None, description="Tool error messages")

    # Performance metrics
    memory_used_mb: float | None = Field(default=None, description="Memory usage in MB")
    cpu_time_ms: float | None = Field(default=None, description="CPU time in milliseconds")
    ...
    tool_specific_data: dict[str, Any] | None = Field(
...
class ToolExecutionSharedData(MutableStrictBaseModel):
    ...
    variables: dict[str, str] = Field(
        default_factory=dict, description="String variables shared between tools"
    )
    files_created: list[str] = Field(
        default_factory=list, description="Files created during execution"
    )
    ...
    working_directory: str | None = Field(default=None, description="Current working directory")
...
class ToolExecutionResult(StrictBaseModel):
    ...
    files_affected: list[str] = Field(
        default_factory=list, description="Files created or modified by this tool"
    )
...
class ToolErrorContext(StrictBaseModel):
    ...
    system_state: str | None = Field(
        default=None, description="Relevant system state information"
    )
...
class ToolExecutionContext(StrictBaseModel):
    ...
    parent_execution_id: str | None = Field(
        default=None, description="Parent execution if nested"
    )
    ...
    safety_checks_enabled: bool = Field(
        default=True, description="Whether safety checks are enabled"
    )
...
class ToolMetadata(StrictBaseModel):
    ...
    planning_description: str | None = Field(
        default=None,
        description="Description for planning prompts (defaults to full description if not provided)",
    )
    tool_category: str = Field(
        default="generic", description="Tool category (generic, software, systems, etc.)"
    )
    version: str = Field(default="1.0.0", description="Tool version")

    # Parameter type enforcement (following flow @pipeline pattern)
    parameter_type: type[ToolParameters] = Field(
        description="Pydantic model for tool parameters (REQUIRED - enforces type safety)"
    )
    ...
    requires_confirmation: bool = Field(
        default=False, description="Whether tool requires user confirmation before execution"
    )
...
class ToolExecutionError(StrictBaseModel):
    ...
    error_code: str | None = Field(default=None, description="Tool-specific error code")
    context: ToolErrorContext = Field(
        default_factory=lambda: ToolErrorContext(operation="unknown"),
...
class AgentTaskRequest(StrictBaseModel):
    ...
    context: RequestContext = Field(
        description="Request context with session, user, and conversation history"
    )
