...
# LLM-facing models (semantic only, no programmatic fields)

class LLMPlanStep(StrictBaseModel):
    """A single step in the execution plan (LLM generates this)."""
    ...
    depends_on_step: Optional[int] = Field(default=None, description="Index of step this depends on (0-based)")


class LLMStructuredPlan(StrictBaseModel):
    """Complete structured plan generated by LLM."""

    message_type: Literal["conversation", "single_tool", "multi_step"] = Field(
        ...,
        description="Type of message: conversation (greeting/question), single_tool (simple task), multi_step (complex task)"
    )
    reasoning: str = Field(..., description="Why this plan was chosen and what it will accomplish")
    steps: List[LLMPlanStep] = Field(..., min_length=1, description="Ordered list of steps to execute - MUST contain at least 1 step")
    expected_outcome: str = Field(..., description="What the user should expect as the result")


# Full models (with programmatic fields)
...
class PlanStep(MutableStrictBaseModel):
    ...
    depends_on_step: Optional[int] = Field(default=None, description="Index of dependent step")
    executed: bool = Field(default=False, description="Whether this step has been executed")
    result: Optional[str] = Field(default=None, description="Result of execution")
...
class StructuredPlan(StrictBaseModel):
    ...
    # Programmatic fields (not from LLM)
...
class PlanningInput(StrictBaseModel):
    ...
    conversation_history: List[dict] = Field(default_factory=list, description="Recent conversation context")
    ...
    shared_variables: Dict[str, str] = Field(
        default_factory=dict,
        description="Shared string variables from execution context"
    )
