...
class VectorMemoryConfig(StrictBaseModel):
    ...
    embedding_dimensions: int | None = Field(
        default=None, description="Embedding dimensions (auto-detected if None)"
    )
    similarity_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum similarity score for search results"
    )
    max_results: int = Field(
        default=50, ge=1, description="Maximum results to return from vector search"
    )
    distance_metric: str = Field(
        default="cosine", description="Vector similarity metric (cosine, euclidean, dot_product)"
    )
...
class VectorMemory:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise MemoryError(
                f"Vector memory initialization failed: {str(e)}", operation="initialize", cause=e
            ) from e
    ...
    async def _ensure_collection(self) -> None:
        ...
        try:
            ...
            collection_exists = await self._vector_provider.index_exists(
    ...
    async def store(self, request: MemoryStoreRequest) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Stored vector memory item '{request.key}' in context '{request.context}'"
            )
            return request.key
    ...
    async def retrieve(self, request: MemoryRetrieveRequest) -> MemoryItem | None:
        ...
        try:
            ...
            results = await self._vector_provider.get_by_filter(
                filter={"context": request.context, "key": request.key},
                top_k=1,
                index_name=self._config.collection_name,
            )
            ...
            if results and len(results) > 0:
                ...
                logger.debug(
                    f"Retrieved vector memory item '{request.key}' from context '{request.context}'"
                )
                return item
            else:
                logger.debug(
                    f"Vector memory item '{request.key}' not found in context '{request.context}'"
                )
                return None

        except Exception as e:
            raise MemoryError(
                f"Vector retrieval failed: {e}", operation="retrieve", key=request.key, cause=e
) from e
    ...
    async def search(self, request: MemorySearchRequest) -> list[MemorySearchResult]:
        ...
        try:
            ...
            logger.debug(
                f"Found {len(search_results)} vector memory results for query '{request.query}'"
            )
            return search_results

        except Exception as e:
            raise MemoryError(f"Vector search failed: {e}", operation="search", cause=e) from e
            ...
            if results and len(results) > 0:
                ...
                if ids_to_delete:
                    await self._vector_provider.delete_vectors(
                        index_name=self._config.collection_name, ids=ids_to_delete
                    )

                    logger.info(
                        f"Wiped vector memory context '{context}', removed {len(ids_to_delete)} items"
                    )
                else:
    ...
    async def _get_embedding(self, text: str) -> list[float]:
        ...
        try:
            ...
            if not isinstance(embeddings[0], list) or not all(
                isinstance(x, (int, float)) for x in embeddings[0]
            ):
                raise MemoryError(
