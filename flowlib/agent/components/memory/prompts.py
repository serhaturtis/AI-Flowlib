...
class FusedMemoryResult(StrictBaseModel):
    ...
    relevant_items: list[str] = Field(
        ..., description="List of relevant items synthesized from different memory sources."
    )
    summary: str = Field(
        ...,
        min_length=1,
        description="A brief summary of the combined findings, acknowledging sources.",
    )
...
class ExtractedKGQueryTerms(StrictBaseModel):
    ...
    terms: list[str] = Field(
        ..., description="List of extracted keywords or entity names suitable for KG search."
    )
