...
class GraphStorageFlow:
    ...
    async def stream_upsert(
        self, extraction_result: DocumentProcessingResult, db_path: str
    ) -> None:
        """Stream upsert entities and relationships to persistent graph database."""

        logger.info(
            f"Streaming upsert: {len(extraction_result.entities)} entities, {len(extraction_result.relationships)} relationships"
        )
        ...
        try:
            ...
            if extraction_result.entities:
                ...
                provider_entities = [
                    self._convert_to_provider_entity(entity)
            ...
            if extraction_result.relationships:
                ...
                logger.debug(
                    f"Streamed {len(extraction_result.relationships)} relationships to graph DB"
                )
    ...
    async def finalize_streaming_graph(self, db_path: str) -> dict:
        ...
        try:
            ...
            return {"status": "finalized", "db_path": db_path, "statistics": stats}
    ...
    async def _get_provider(self, config: GraphStoreInput) -> GraphDBProvider:
        ...
        logger.info(f"Initializing graph provider: {config.graph_provider_config}")
    ...
    def _convert_to_provider_relationship(
        self, knowledge_rel: Relationship
    ) -> "EntityRelationship":
    ...
    def _convert_to_graph_entity(self, entity: Entity) -> GraphEntity:
        ...
        attributes.append(
            GraphEntityAttribute(name="entity_type", value=entity.entity_type.value, type="string")
        )

        if entity.description:
            attributes.append(
                GraphEntityAttribute(name="description", value=entity.description, type="string")
            )

        # Add confidence and frequency
        attributes.append(
            GraphEntityAttribute(name="confidence", value=entity.confidence, type="float")
        )

        attributes.append(
            GraphEntityAttribute(name="frequency", value=entity.frequency, type="integer")
        )

        # Add document count
        attributes.append(
            GraphEntityAttribute(name="document_count", value=len(entity.documents), type="integer")
        )
    ...
    async def _build_graph(self, input_data: GraphStoreInput) -> GraphStoreOutput:
        ...
        logger.info(
            f"Building graph with {len(input_data.entities)} entities and {len(input_data.relationships)} relationships"
        )
        ...
        try:
            ...
            for doc in input_data.documents:
                GraphEntity(
                    ...
                    attributes=[
                        GraphEntityAttribute(
                            name="file_type", value=doc.metadata.file_type, type="string"
                        ),
                        GraphEntityAttribute(
                            name="file_size", value=doc.metadata.file_size, type="integer"
                        ),
                        GraphEntityAttribute(
                            name="created_date",
                            value=doc.metadata.created_date or "",
                            type="string",
                        ),
                        GraphEntityAttribute(
                            name="language", value=doc.language_detected, type="string"
                        ),
                        GraphEntityAttribute(name="status", value=doc.status.value, type="string"),
                        GraphEntityAttribute(
                            name="chunk_count", value=len(doc.chunks), type="integer"
                        ),
                        GraphEntityAttribute(
                            name="reading_time", value=doc.reading_time_minutes, type="float"
                        ),
                    ],
            ...
            for entity in input_data.entities:
                ...
                entity_nodes.append(
                    GraphNode(
                        ...
                        name=entity.name,
                        properties={"confidence": entity.confidence, "frequency": entity.frequency},
                    )
                )
                ...
                edges.append(
                    GraphEdge(
                        ...
                        properties={
                            ...
                            or relationship.relationship_type.value,
                        },
                    )
                )
            ...
            graph_stats = GraphStatistics(
                ...
                node_types={"documents": len(doc_nodes), "entities": len(entity_nodes)},
                relationship_types={rel.relationship_type: 1 for rel in edges},  # Simplified count
                average_degree=2.0 * len(edges) / max(len(entity_nodes), 1)
                if entity_nodes
                else 0.0,
            )
    ...
    async def _query_graph(self, input_data: GraphStoreInput) -> GraphStoreOutput:
        ...
        try:
            ...
            if input_data.query_entity_id:
                ...
                entity_search_result = await graph_provider.search_entities(
                    entity_type=input_data.query_entity_type, limit=input_data.query_limit or 100
                )
            ...
            basic_stats = GraphStatistics(total_nodes=len(nodes), total_edges=len(edges))
    ...
    async def _find_connections(self, input_data: GraphStoreInput) -> GraphStoreOutput:
        ...
        logger.info(
            f"Finding connections between {input_data.query_source_id} and {input_data.query_target_id}"
        )
        ...
        try:
            ...
            relationships = await graph_provider.query_relationships(
                entity_id=input_data.query_source_id, direction="outgoing"
            )
            ...
            for rel in relationships:
                ...
                if source_id not in seen_nodes:
                    ...
                    nodes.append(
                        GraphNode(
                            node_id=source_id, node_type="entity", name=source_label, properties={}
                        )
                    )
                ...
                if target_id not in seen_nodes:
                    ...
                    nodes.append(
                        GraphNode(
                            node_id=target_id, node_type="entity", name=target_label, properties={}
                        )
                    )
            ...
            path_stats = GraphStatistics(total_nodes=len(nodes), total_edges=len(edges))
