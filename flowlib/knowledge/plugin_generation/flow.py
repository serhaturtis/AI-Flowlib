...
class PluginGenerationFlow:
    ...
    async def run_pipeline(self, request: PluginGenerationRequest) -> PluginGenerationResult:
        ...
        try:
            ...
            domain_strategy = domain_strategy_registry.get_strategy(
                request.domain_strategy, request.domain_config
            )
            ...
            try:
                ...
                extraction_stats = await self._run_knowledge_extraction(
                    request=request, temp_dir=temp_dir
                )
                ...
                await self._create_database_configs(
                    plugin_dir, request.plugin_name, request.use_vector_db, request.use_graph_db
                )
                await self._create_embedded_databases(
                    plugin_dir,
                    temp_dir,
                    request.plugin_name,
                    request.use_vector_db,
                    request.use_graph_db,
                )
                ...
                return PluginGenerationResult(
                    success=True, plugin_path=str(plugin_dir), summary=summary
                )
    ...
    async def _run_knowledge_extraction(
        self, request: PluginGenerationRequest, temp_dir: Path
    ) -> dict[str, Any]:
        ...
        # Get extraction flow from registry
        flow = flow_registry.get_flow("knowledge-extraction")
        ...
        try:
            ...
            raise RuntimeError(f"Knowledge extraction pipeline failed: {e}") from e
    ...
    async def _process_extraction_results(self, temp_dir: Path) -> ProcessedData:
        ...
        if not data_dir.exists():
            raise RuntimeError(f"Invalid plugin structure - missing data directory: {data_dir}")
        ...
        try:
            ...
            logger.info(
                f"Loaded extraction data: {len(processed_data.documents)} docs, {len(processed_data.entities)} entities, {len(processed_data.relationships)} relationships"
            )

        except Exception as e:
            raise RuntimeError(f"Failed to load extraction data from {data_dir}: {e}") from e
    ...
    async def _generate_plugin_files(
        ...
        await self._generate_manifest(plugin_dir, request, extraction_stats)

        # Generate provider.py
        await self._generate_provider(plugin_dir, request, processed_data)

        # Generate __init__.py
        await self._generate_init_file(plugin_dir, request)

        # Generate README.md
        await self._generate_readme(plugin_dir, request, extraction_stats)
    ...
    async def _generate_manifest(
        self, plugin_dir: Path, request: PluginGenerationRequest, extraction_stats: dict[str, Any]
    ) -> None:
        ...
        manifest_data = {
            ...
            else f"Knowledge plugin for {', '.join(request.domains)}",
    ...
    async def _generate_provider(
        self, plugin_dir: Path, request: PluginGenerationRequest, processed_data: ProcessedData
    ) -> None:
...
'''
    ...
    async def _generate_test_script(
        self, plugin_dir: Path, request: PluginGenerationRequest
    ) -> None:
...
'''
    ...
    async def _create_database_configs(
        self, plugin_dir: Path, plugin_name: str, use_vector_db: bool, use_graph_db: bool
    ) -> None:
    ...
    async def _create_embedded_databases(
        self,
        plugin_dir: Path,
        temp_dir: Path,
        plugin_name: str,
