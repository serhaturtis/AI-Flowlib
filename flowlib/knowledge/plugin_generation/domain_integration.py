...
from ..models import ChunkingStrategy, ExtractionConfig
from .domain_strategies import BaseDomainStrategy, domain_strategy_registry
...
class DomainAwarePluginGenerator:
    ...
    def get_domain_strategy(self, request: PluginGenerationRequest) -> BaseDomainStrategy:
        ...
        return self.registry.get_strategy(request.domain_strategy, request.domain_config)

    def create_domain_aware_extraction_config(
        self, request: PluginGenerationRequest, domain_strategy: BaseDomainStrategy
    ...
    def validate_extraction_results(
        self, domain_strategy: BaseDomainStrategy, extraction_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if validation_errors:
            ...
            logger.info(
                "✅ Domain validation passed - all extracted data conforms to domain expectations"
            )
    ...
    def enhance_plugin_metadata(
        ...
        logger.info(
            f"Enhanced plugin metadata with domain strategy: {domain_strategy.strategy_name}"
        )
    ...
    def get_available_strategies(self) -> dict[str, dict[str, str]]:
        ...
        return {str(domain): metadata for domain, metadata in strategies.items()}
