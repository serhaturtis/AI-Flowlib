...
class DomainStrategyConfig(StrictBaseModel):
    ...
    custom_entity_types: list[str] | None = Field(
        default=None, description="Custom entity types for this domain"
    )
    custom_relationship_types: list[str] | None = Field(
        default=None, description="Custom relationship types"
    )
    extraction_focus: list[str] | None = Field(
        default=None, description="Areas to focus extraction on"
    )
    validation_rules: dict[str, Any] | None = Field(
        default=None, description="Domain-specific validation rules"
    )
...
class DatabaseSettings(StrictBaseModel):
    ...
    chromadb: DatabaseConfig | None = Field(default=None, description="ChromaDB configuration")
    neo4j: DatabaseConfig | None = Field(default=None, description="Neo4j configuration")
...
class PluginGenerationRequest(StrictBaseModel):
    ...
    domain_strategy: DomainStrategy = Field(
        default=DomainStrategy.GENERIC, description="Domain-specific generation strategy"
    )
    domain_config: DomainStrategyConfig | None = Field(
        default=None, description="Domain strategy configuration"
    )
