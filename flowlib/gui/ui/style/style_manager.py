...
from .resources import ResourceGenerator, RESOURCES_PATH
...
class StyleManager:
    ...
    def export_theme(
        ...
        stylesheet = StyleManager.build_stylesheet(
    ...
    def build_stylesheet(
        ...
        if not export:
            try:
                StyleManager.add_fonts()
            except Exception as e:
                logging.warning(e)
    
        theme = StyleManager.get_theme(theme, invert_secondary)
        if theme is None:
            return None
    
        StyleManager.set_icons_theme(theme, parent=parent)
    
        # Render custom template
        if os.path.exists(template):
            parent, template_name = os.path.split(template)
            loader = jinja2.FileSystemLoader(parent)
            env = jinja2.Environment(autoescape=False, loader=loader)
            env.filters['opacity'] = StyleManager.opacity
            env.filters['density'] = StyleManager.density
        ...
        if GUI:
            ...
            try:
                if hasattr(
                    ...
                    default_palette.setColor(QPalette.ColorRole.PlaceholderText, color)
                else:  # pyqt6
    ...
    def get_theme(theme_name, invert_secondary=False):
        ...
        document = xml.dom.minidom.parse(theme)
        theme = {
    ...
    def apply_stylesheet(
        ...
        if style:
            try:
                ...
                logging.error(f"Failed to apply style '{style}': {style_error}")
        ...
        stylesheet = StyleManager.build_stylesheet(theme, invert_secondary, extra, parent)
        ...
        try:
            ...
            # PySide6 compatibility: try snake_case version  
...
#
#        for i, theme in enumerate(['default'] + StyleManager.list_themes()):
