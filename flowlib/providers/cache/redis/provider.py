...
class RedisCacheProviderSettings(CacheProviderSettings):
    ...
    host: str = Field(
        default="localhost",
        description="Redis server host (e.g., 'localhost', 'redis.example.com')",
    )
    ...
    username: str | None = Field(
        default=None, description="Redis username for ACL auth (Redis 6.0+)"
    )
    password: str | None = Field(
        default=None, description="Redis password (e.g., 'mySecurePassword123')"
    )
    ...
    socket_timeout: float | None = Field(default=None, description="Socket timeout in seconds")
    socket_connect_timeout: float | None = Field(
        default=None, description="Socket connection timeout in seconds"
    )
    ...
    socket_keepalive_options: dict[int, int | bytes] | None = Field(
        default=None, description="Socket keepalive options"
    )
    ...
    decode_responses: bool = Field(
        default=False, description="Whether to decode responses to strings"
    )

    # Redis Sentinel settings (for high availability)
    sentinel_kwargs: dict[str, Any] = Field(
        default_factory=dict, description="Additional options for Redis Sentinel"
    )
    sentinel: list[str] | None = Field(default=None, description="List of Redis Sentinel nodes")
    sentinel_master: str | None = Field(
        default=None, description="Name of the Redis Sentinel master"
    )
...
class RedisCacheProvider(CacheProvider[RedisCacheProviderSettings]):
    ...
    def __init__(
        self, name: str = "redis-cache", settings: RedisCacheProviderSettings | None = None
    ):
    ...
    async def clear(self) -> bool:
        ...
        try:
            ...
            raise ProviderError(
                ...
                provider_context=ProviderErrorContext(
                    provider_name=self.name, provider_type="cache", operation="clear", retry_count=0
                ),
                cause=e,
) from e
        except Exception as e:
