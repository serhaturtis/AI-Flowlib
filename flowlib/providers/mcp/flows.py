...
class MCPRegistry:
    ...
    def get_client(self, name: str) -> MCPClientProvider | None:
        """Get an MCP client by name."""
        return self._clients.get(name)

    def get_tool(self, client_name: str, tool_name: str) -> MCPTool | None:
...
class MCPToolExecutorFlow:
    ...
    async def run_pipeline(self, input_data: MCPToolExecutionInput) -> MCPToolExecutionOutput:
        ...
        try:
            ...
            logger.error(
                f"Error executing MCP tool '{input_data.tool_name}' on client '{input_data.client_name}': {e}"
            )
            return MCPToolExecutionOutput(
                success=False,
                result=None,
