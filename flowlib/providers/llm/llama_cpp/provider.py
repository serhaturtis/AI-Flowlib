...
class LlamaCppProvider(LLMProvider[LlamaCppSettings]):
    ...
    async def generate_structured(
        ...
        # DEBUG: Show final formatted prompt with chat template
        ...
        # if sanitized_text != generated_text:
        print("\n🧹 SANITIZED TEXT:")
        print("=" * 80)
        ...
        try:
            ...
            # DEBUG: Show final validated response
            print(f"\n✅ VALIDATED RESPONSE ({output_type.__name__}):")
            print("=" * 80)
    ...
    def _format_prompt(
        ...
        prompt_with_json = super()._format_prompt(prompt, chat_format, output_type)

        # Get template for model type with strict lookup
        templates = self._get_model_templates()
        chat_format_key = chat_format.lower()

        if chat_format_key not in templates:
            ...
            template = templates[chat_format_key]
