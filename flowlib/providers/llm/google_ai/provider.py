...
class GoogleAISettings(LLMProviderSettings):
    ...
    api_key: str = Field(
        default="",
        description="Google AI API key (e.g., 'AIzaSyBJDV...', get from Google AI Studio)",
    )
    api_base: str | None = Field(default=None, description="Custom API base URL (optional)")

    # Google AI safety controls
    safety_settings: dict[str, object] | None = Field(
        default=None, description="Safety settings for content generation filtering"
    )
    ...
    requests_per_minute: int = Field(
        default=5, description="Max requests per minute (conservative default)"
    )
    enable_rate_limiting: bool = Field(default=True, description="Enable built-in rate limiting")
    min_request_interval: float = Field(
        default=12.0, description="Minimum seconds between requests (conservative)"
    )
    ...
    rate_limit_backoff_factor: float = Field(
        default=1.5, description="Backoff multiplier for 429 retries"
    )
    rate_limit_max_backoff: float = Field(
        default=120.0, description="Maximum backoff delay in seconds"
    )
...
class GoogleAIProvider(LLMProvider[GoogleAISettings]):
    ...
    _settings: GoogleAISettings = PrivateAttr()
    _client: Optional["genai.Client"] = PrivateAttr(default=None)
    # Note: _models is already declared in LLMProvider but we override the type hint
    _last_request_time: float | None = PrivateAttr(default=None)
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        if not isinstance(self.settings, GoogleAISettings):
            raise TypeError(
                f"settings must be a GoogleAISettings instance, got {type(self.settings)}"
            )

        # Set private attributes normally (they're mutable with PrivateAttr)
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self._settings.enable_rate_limiting:
                logger.info(
                    f"Rate limiting enabled: {self._settings.requests_per_minute} requests/minute, {self._settings.min_request_interval}s minimum interval"
                )
    ...
    async def _initialize(self) -> None:
        ...
        if not self._initialized:
            ...
            logger.warning(
                "GoogleAIProvider._initialize() called but provider not yet initialized. Call initialize() first."
            )
    ...
    def _get_or_cache_model_config(self, model_name: str, model_config: dict[str, object]) -> str:
        ...
        if model_name not in self._models:
            ...
            if "model_id" not in model_config:
                raise ValueError(
                    f"Model configuration for '{model_name}' must specify 'model_id' (e.g., 'gemini-2.0-flash-001')"
                )
        ...
        **kwargs: Any,
        ...
        for attempt in range(self._settings.rate_limit_retry_attempts):
            ...
            try:
                ...
                logger.info(
                    f"Rate limit retry succeeded for {operation_name} after {backoff_delay:.1f}s backoff"
                )
                return result
            except Exception as e:
                ...
                if attempt == self._settings.rate_limit_retry_attempts - 1:
                    ...
                    logger.error(
                        f"Rate limit retry exhausted for {operation_name} after {self._settings.rate_limit_retry_attempts} attempts"
                    )
                    raise e
    ...
    async def _execute_with_rate_limiting(
        ...
        try:
            ...
            if self._is_rate_limit_error(e):
                ...
                return await self._handle_rate_limit_retry(
                    operation_name, operation, e, *args, **kwargs
                )
            raise
    ...
    async def generate(
        ...
        try:
            ...
            if "config" not in model_registry_config_raw or not isinstance(
                model_registry_config_raw["config"], dict
            ):
                raise ValueError(
            ...
            logger.warning(
                f"Could not retrieve model config for '{model_name}': {e}. Using defaults."
            )
        ...
        logger.info(
            f"Generating text with Gemini model '{gemini_model_id}' (registry: '{model_name}')"
        )
        ...
        try:
            ...
            async def _generation_operation() -> Any:
                ...
                return self._client.models.generate_content(
                    model=gemini_model_id, contents=final_prompt_content, config=generation_config
                )

            response = await self._execute_with_rate_limiting(
                f"generate_{model_name}", _generation_operation
            )
            # Safety ratings check (optional, for logging or specific handling)
            if response.prompt_feedback and response.prompt_feedback.block_reason:
                logger.warning(
                    f"Prompt blocked for model '{gemini_model_id}'. Reason: {response.prompt_feedback.block_reason_message}"
                )
                error_context = ErrorContext.create(
                    flow_name="GoogleAIProvider",
            ...
            if not response.candidates or not response.candidates[0].content.parts:
                logger.warning(
                    f"No content generated by model '{gemini_model_id}'. Response: {response}"
                )
    ...
    async def generate_structured(
        ...
        logger.info(
            f"Generating structured output with Gemini model '{gemini_model_id}' (registry: '{model_name}')"
        )
        ...
        try:
            ...
            response = await self._execute_with_rate_limiting(
                f"generate_structured_{model_name}", _structured_generation_operation
            )

            if response.prompt_feedback and response.prompt_feedback.block_reason:
                logger.warning(
                    f"Prompt blocked for model. Reason: {response.prompt_feedback.block_reason_message}"
                )
                error_context = ErrorContext.create(
                    flow_name="GoogleAIProvider",
                    error_type="BlockedPromptError",
            ...
            try:
                ...
                logger.error(
                    f"JSON parsing failed for structured output: {parse_err}. Response text: {response.text[:200]}"
                )
                error_context = ErrorContext.create(
                    flow_name="GoogleAIProvider",
