...
class RabbitMQProviderSettings(MQProviderSettings):
    ...
    connection_string: str | None = Field(
        default=None, description="Connection string (overrides host/port if provided)"
    )
    ...
    ssl_options: dict[str, Any] | None = Field(default=None, description="SSL options")

    # Exchange and queue settings
    exchange_name: str | None = Field(default=None, description="Exchange name")
    exchange_type: str = Field(
        default="topic", description="Exchange type (direct, fanout, topic, headers)"
    )
...
class RabbitMQProvider(MQProvider[RabbitMQProviderSettings]):
    ...
    def __init__(self, name: str = "rabbitmq", settings: RabbitMQProviderSettings | None = None):
        ...
        if not isinstance(self.settings, RabbitMQProviderSettings):
            raise TypeError(
                f"settings must be a RabbitMQProviderSettings instance, got {type(self.settings)}"
            )
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Connected to RabbitMQ: {self.settings.host}:{self.settings.port}/{self.settings.virtual_host}"
            )
            ...
            raise ProviderError(
                ...
                provider_context=ProviderErrorContext(
                    provider_name=self.name, provider_type="mq", operation="connect", retry_count=0
                ),
                cause=e,
) from e
    ...
    async def _shutdown(self) -> None:
        ...
        try:
            ...
            if self._connection:
                ...
                logger.info(
                    f"Closed RabbitMQ connection: {self.settings.host}:{self.settings.port}"
                )
        except Exception as e:
    ...
    async def publish(
        ...
        try:
            ...
            await self._exchange.publish(message=amqp_message, routing_key=routing_key)
    ...
    async def consume(
        ...
        try:
            ...
            async def message_handler(message: AbstractIncomingMessage) -> None:
                ...
                ):  # Use ignore_processed for safety
                    try:
                        # Get message body
                        body = message.body
                        ...
                        if message.content_type == "application/json":
                            try:
                                ...
                                logger.warning(
                                    f"Failed to decode JSON, passing raw data. Msg ID: {message.message_id}"
                                )
                        ...
                        logger.exception(
                            f"Error processing message (ID: {message.message_id}): {str(e)}"
                        )
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to NACK message (ID: {message.message_id}): {nack_err}"
                            )
            ...
            raise ProviderError(
                ...
                provider_context=ProviderErrorContext(
                    provider_name=self.name, provider_type="mq", operation="consume", retry_count=0
                ),
                cause=e,
) from e
    ...
    async def stop_consuming(self, consumer_tag: str) -> None:
        ...
        if not queue_name:
            logger.error(
                f"Could not find queue name for consumer tag '{consumer_tag}'. Cannot cancel."
            )
            # Clean up tag set
        ...
        if not queue:
            logger.error(
                f"Could not find queue object for name '{queue_name}' (tag: '{consumer_tag}'). Cannot cancel."
            )
            # Clean up maps
            self._consumer_tags.discard(consumer_tag)
            del self._tag_to_queue_map[consumer_tag]
        ...
        try:
            ...
            logger.info(
                f"Stopped consuming for consumer tag '{consumer_tag}' on queue '{queue_name}'"
            )
        except Exception as e:
            logger.error(
                f"Error cancelling consumer tag '{consumer_tag}' on queue '{queue_name}': {e}",
                exc_info=True,
            )
    ...
    async def create_queue(
        ...
        try:
            ...
            queue_auto_delete = (
                auto_delete if auto_delete is not None else self.settings.queue_auto_delete
            )
            ...
            queue = await self._channel.declare_queue(
                name=queue_name, durable=queue_durable, auto_delete=queue_auto_delete
            )
