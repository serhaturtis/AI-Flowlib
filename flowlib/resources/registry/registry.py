...
class ResourceRegistry(BaseRegistry[ResourceBase]):
    ...
    def register(
        ...
        name: str,
        obj: ResourceBase,
        resource_type: str = ResourceType.MODEL_CONFIG,
        **metadata: str | int | bool,
        ...
        if resource_type not in known_types and not resource_type.startswith(("type", "test")):
            raise ValueError(
                f"Invalid resource type '{resource_type}'. Must be one of: {known_types}"
            )

        key = (resource_type, name)
        if not isinstance(obj, ResourceBase):
            raise TypeError(
                f"Resource '{name}' must be a ResourceBase subclass (pydantic v2), got {type(obj)}"
            )
    ...
    def get(self, name: str, expected_type: type[Any] | None = None) -> ResourceBase:
        ...
        for (_resource_type, resource_name), resource in self._resources.items():
    ...
    def get_metadata(self, name: str) -> dict[str, Any]:
        ...
        for (resource_type, resource_name), _resource in self._resources.items():
            if resource_name == name:
    ...
    def contains(self, name: str) -> bool:
        ...
        """
        # Search across all resource types for the name
        for (_resource_type, resource_name), _resource in self._resources.items():
            if resource_name == name:
                return True
        return False
    ...
    def list_types(self) -> builtins.list[str]:
